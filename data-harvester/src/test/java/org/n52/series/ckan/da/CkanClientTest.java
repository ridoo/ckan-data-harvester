...
public class CkanClientTest {
    ...
    public void getAllDatasets() {
        LOGGER.info("#### all dataset names available");
        CkanClient client = new CkanClient("https://ckan.colabis.de/");
        List<CkanDataset> datasetList = client.searchDatasets(CkanQuery.filter(), 20, 0).getResults();
        for (CkanDataset dataset : datasetList) {
            LOGGER.info("Dataset ##############################");
            LOGGER.info("id: {}", dataset.getId());
            LOGGER.info("name: {}", dataset.getName());
            LOGGER.info("Organization name: {}", dataset.getOrganization().getName());
            LOGGER.info("Organization title: {}", dataset.getOrganization().getTitle());
        }
    }
